...
class SettingsDialog(QObject):
    """Python backend for the Settings dialog."""
    ...
    def show(self):
        ...
        if self._component is None:
            self._component = QQmlComponent(self._engine, "qrc:/qml/settings.qml")
        ...
        if self._component.status() == QQmlComponent.Status.Ready:
            ...
            print(f"Settings dialog component error: {self._component.errors()}")
    ...
    def _create_window(self):
        ...
        if self._window is None:
            print(f"Failed to create Settings dialog: {self._component.errors()}")
            return
        self._window.show()
        self._window.raise_()
        self._window.requestActivate()
